...
gulp.task("images", function() {
  return gulp
    .src(join(IMAGES, "**/*.+(png|jpg|jpeg|gif|svg)"))
...
gulp.task("default", function(callback) {
  runSequence("clean", ["templates", "less", "js", "images"], callback);
});
...
gulp.task("deploy:theme", function() {
  gulp.src(DEST).pipe(rsync(deployTo("public_html/wp-content/themes")));
});

gulp.task("deploy:resources", function() {
  gulp.src(RESOURCES).pipe(rsync(deployTo("public_html")));
});
